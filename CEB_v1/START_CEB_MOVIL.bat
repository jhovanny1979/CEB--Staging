...
if "%IS_ADMIN%"=="1" (
  echo [INFO] Preparando red local para modo movil...
  ...
  echo [OK] Perfil de red revisado.
) else (
...
if "%IS_ADMIN%"=="1" (
  netsh advfirewall firewall delete rule name="CEB Frontend %FRONT_PORT%" >nul 2>&1
  netsh advfirewall firewall delete rule name="CEB Backend %BACK_PORT%" >nul 2>&1
  netsh advfirewall firewall add rule name="CEB Frontend %FRONT_PORT%" dir=in action=allow protocol=TCP localport=%FRONT_PORT% profile=any >nul 2>&1
  netsh advfirewall firewall add rule name="CEB Backend %BACK_PORT%" dir=in action=allow protocol=TCP localport=%BACK_PORT% profile=any >nul 2>&1
  echo [OK] Reglas de firewall aplicadas para puertos %FRONT_PORT% y %BACK_PORT%.
)
...
if exist "%CAP_CONFIG%" if exist "%UPDATE_PS1%" (
  for /f "usebackq delims=" %%i in (`powershell -NoProfile -ExecutionPolicy Bypass -File "%UPDATE_PS1%" -ConfigPath "%CAP_CONFIG%" -PreferredInterfaceAlias "%PREFERRED_IF%" -HostOverride "%LAN_IP%" -FrontPort %FRONT_PORT% -BackendPort %BACK_PORT%`) do set "APK_IP=%%i"
  if not "%APK_IP%"=="" (
    echo [INFO] Configuracion APK actualizada para IP: %APK_IP% (front %FRONT_PORT%, api %BACK_PORT%)
  )
)
...
echo      http://%LAN_IP%:%BACK_PORT%/health/live
echo.
echo [INFO] Validando acceso local por IP LAN detectada...
powershell -NoProfile -Command "try { $r=Invoke-WebRequest -UseBasicParsing -TimeoutSec 4 'http://%LAN_IP%:%FRONT_PORT%/index.html?app=1'; if($r.StatusCode -eq 200){ Write-Host '[OK] Frontend LAN responde en http://%LAN_IP%:%FRONT_PORT%/index.html?app=1' } else { Write-Host '[WARN] Frontend LAN respondio con codigo inesperado' } } catch { Write-Host ('[ERROR] Frontend LAN no responde: ' + $_.Exception.Message) }"
powershell -NoProfile -Command "try { $r=Invoke-WebRequest -UseBasicParsing -TimeoutSec 4 'http://%LAN_IP%:%BACK_PORT%/health/live'; if($r.StatusCode -eq 200){ Write-Host '[OK] Backend LAN responde en http://%LAN_IP%:%BACK_PORT%/health/live' } else { Write-Host '[WARN] Backend LAN respondio con codigo inesperado' } } catch { Write-Host ('[ERROR] Backend LAN no responde: ' + $_.Exception.Message) }"
echo.
