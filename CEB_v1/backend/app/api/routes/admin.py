...
def set_admin_user_password(
    ...
    db.commit()
    return MessageResponse(message='Contrasena de administrador actualizada')


@router.post('/clients/{user_id}/set-password', response_model=MessageResponse)
def set_client_user_password(
    ...
    target = db.get(User, user_id)
    if not target or target.role != RoleEnum.client:
    ...
    log_action(
        db,
        actor_user_id=admin.id,
        action='admin_client_password_update',
        entity_type='user',
        entity_id=str(target.id),
        metadata={'target_role': target.role.value, 'target_email': target.email},
    )
    db.commit()
    return MessageResponse(message='Contrasena de cliente actualizada')
