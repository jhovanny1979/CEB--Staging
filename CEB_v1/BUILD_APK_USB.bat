...
echo [INFO] Modo USB: configurando APK para 127.0.0.1...
for /f "usebackq delims=" %%i in (`powershell -NoProfile -ExecutionPolicy Bypass -File "%UPDATE_PS1%" -ConfigPath "%CONFIG_JSON%" -HostOverride "127.0.0.1"`) do set "APP_HOST=%%i"
if errorlevel 1 (
  echo [ERROR] No se pudo actualizar capacitor.config.json para modo USB
  exit /b 1
)
if "%APP_HOST%"=="" set "APP_HOST=127.0.0.1"
echo [INFO] Host app configurado: %APP_HOST%
...
echo.
echo [SIGUIENTE PASO]
echo 1) INSTALL_APK.bat
echo 2) START_CEB_MOVIL_USB.bat
echo.
echo [TIP] La APK abre un lanzador interno y ya quedara con host sugerido 127.0.0.1.
